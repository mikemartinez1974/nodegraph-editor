...
class NodeGraphEditorProvider {
    ...
    async resolveOutRoot(documentUri) {
        const candidates = new Set();
        const addCandidate = (fsPath) => {
            if (fsPath) {
    ...
    async getHtml(webview, outRoot, documentUri) {
        ...
        try {
            ...
            if (turbopackMatch) {
                ...
                if (runtimeRegex.test(runtimeText)) {
                    runtimeText = runtimeText.replace(runtimeRegex, `$1 t=${JSON.stringify(runtimePrefix)}`);
                    ...
                    if (runtimeText.includes(runtimeUrlToken)) {
                        runtimeText = runtimeText.replace(runtimeUrlToken, 'function S(e){return/^[a-zA-Z][a-zA-Z0-9+.-]*:/.test(e)?e:`${t}${e.replace(/^\\/_next\\//,"").split("/").map(e=>encodeURIComponent(e)).join("/")}`}');
                    }
                    if (runtimeText.includes(currentScriptToken)) {
                        runtimeText = runtimeText.replace(currentScriptToken, `[${JSON.stringify(runtimeChunkPath)},`);
                    }
                    ...
                    const patchedSrc = turbopackSrc.startsWith('/')
                        ? `/${patchedRuntimePath}`
                        : patchedRuntimePath;
                    html = html.replace(turbopackMatch[0], `<script src="${patchedSrc}"></script>`);
                }
            }
            ...
            while ((scriptMatch = scriptTagRegex.exec(html))) {
                ...
                if (!scriptSrc.includes('/_next/static/chunks/') || scriptSrc.includes('.vscode.js'))
                    continue;
                ...
                if (scriptText.includes(currentScriptToken)) {
                    ...
                    scriptText = scriptText.replace(currentScriptToken, `[${JSON.stringify(chunkPath)},`);
                    patched = true;
                }
                if (scriptText.includes('getAssetPrefix') && scriptText.includes('document.currentScript')) {
                    const getAssetPrefixRegex = /(Object\.defineProperty\(n,"getAssetPrefix"[\s\S]*?)function l\(\)\{[\s\S]*?return t\.slice\(0,n\)\}/;
...
NodeGraphEditorProvider.viewType = 'twilight.nodeGraph';
function activate(context) {
    context.subscriptions.push(vscode.window.registerCustomEditorProvider(NodeGraphEditorProvider.viewType, new NodeGraphEditorProvider(context), { supportsMultipleEditorsPerDocument: false }));
}
function deactivate() { }
function getNonce() {
    let text = '';
