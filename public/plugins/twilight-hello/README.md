...
- `definition.ports` seeds the node with an input trigger and greeting output.
...
`canvasRenderer.js` shows how to render a fully custom canvas UI inside the sandbox. It draws a waveform/barchart directly on a `<canvas>` element so the plugin node looks identical to the native `CanvasNode` (no default chrome, ports pinned close to the content).
