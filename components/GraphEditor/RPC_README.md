...
- `url` (string): URL to load in the sandboxed iframe.
- `interactive` (boolean): Enable pointer events for the iframe (default: `false`).
- `allowedOrigins` (string[]): Whitelist of permitted iframe origins. Defaults to the current origin; include `"null"` for data/blob URLs.
- `onHandshakeComplete` (function): Called when the iframe completes the RPC handshake with the list of exposed methods.
...
- Every handshake and RPC message is stamped with a per-session token generated by the host. Messages without the correct token are rejected.
- Origins are validated before processing messages. Configure `allowedOrigins` to the minimal set required for your deployment.
- Timeouts automatically reject unresolved RPC calls to avoid resource leaks.
