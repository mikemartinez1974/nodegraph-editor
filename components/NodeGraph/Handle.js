...
const HANDLE_OFFSET = 24; // pixels from node center to lower right (adjust as needed)
...
export const Handle = React.memo(function Handle({ cx, cy, color, isDragging, handle }) {
  ...
  // Position handle at lower right of node
  const handleCx = cx + HANDLE_OFFSET;
  const handleCy = cy + HANDLE_OFFSET;
  ...
  return (
    <div
      ...
      style={{
        position: 'absolute',
        left: `${handleCx - HANDLE_RADIUS}px`,
        top: `${handleCy - HANDLE_RADIUS}px`,
