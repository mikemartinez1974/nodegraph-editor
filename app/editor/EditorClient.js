...
console.log('[TwiliteEditor] client module loaded');
...
export default function EditorClient() {
  const isEmbedded =
    ...
    (window.location?.search?.includes('embed=1') || window.__Twilite_EMBED__ === true);
  ...
  useEffect(() => {
    ...
    window.__Twilite_EDITOR_PAGE__ = true;
    console.log('[TwiliteEditor] page mounted', { isEmbedded });
  }, [isEmbedded]);
  useEffect(() => {
    if (typeof window === 'undefined') return;
  ...
  useEffect(() => {
    ...
    const handleReady = () => {
      try {
        postHostMessage({ type: 'rendererReady' });
      } catch (err) {}
    };
